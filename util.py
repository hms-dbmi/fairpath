...
def image_transform(size=224):
    return albu.Compose([
        albu.PadIfNeeded(min_height=256, min_width=256,
                         border_mode=cv2.BORDER_CONSTANT, value=(0, 0, 0), p=1),
        albu.HorizontalFlip(p=0.3),
        albu.RandomRotate90(p=0.6),
        albu.CoarseDropout(max_holes=60, max_height=32, max_width=32,
                           min_holes=10, min_height=8, min_width=8, fill_value=0, p=0.5),
        albu.Sequential([
            albu.ColorJitter(brightness=0.1, saturation=0.3,
                             contrast=0.3, hue=0, p=0.4)
        ], p=1),
...
#### Function below are not used in the upcoming version ####
