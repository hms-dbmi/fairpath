...
#SBATCH -o ./logs/tile_TCGA_GBM_PM/WSI_tile%A_%a.log                 # File to which STDOUT will be written, including job ID (%j)
#SBATCH -e ./logs/tile_TCGA_GBM_PM/WSI_tile%A_%a.log                 # File to which STDERR will be written, including job ID (%j)
#SBATCH --array=0-859          # Run array for indexes. Should set to the length of the file list
...
FILE_LIST="data_sheet/TCGA_GBM_PM(N=860).csv"
...
TILE_PARAMS="step1_tile_extraction/tiling_params/tile_params_quick500_w512s512_5X_Sophie.jsonc"        ## tile param file
PROJ="TCGA_GBM_PM"                ## output subfolder name. the tiles will be stored in [outpath]/[proj]/[slide_name]

python step1_tile_extraction/WSI_tile_extraction_batch_sophie.py \
 --params ${TILE_PARAMS} \
