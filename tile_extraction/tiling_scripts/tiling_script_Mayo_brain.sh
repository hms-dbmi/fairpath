...
#SBATCH -o ./logs/Mayo_GBM_LGG/WSI_tile%A_%a.log                 # File to which STDOUT will be written, including job ID (%j)
#SBATCH -e ./logs/Mayo_GBM_LGG/WSI_tile%A_%a.log                 # File to which STDERR will be written, including job ID (%j)
#SBATCH --array=0-165          # Run array for indexes. Should set to the length of the file list
...
FILE_LIST="/n/data2/hms/dbmi/kyu/lab/shl968/fairness_external_validation/data_sheet/Mayo_GBM_LGG(N=166).csv"
OUTPATH="/n/data2/hms/dbmi/kyu/lab/shl968/tile_datasets"    ## main output folder for extracted tiles
TILE_PARAMS="step1_tile_extraction/tiling_params/tile_params_quick500_w256s256_satOtsu.jsonc"        ## tile param file
PROJ="Mayo_GBM_LGG"                ## output subfolder name. the tiles will be stored in [outpath]/[proj]/[slide_name]
