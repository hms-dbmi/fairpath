...
#SBATCH -o ./logs/tile_DFCI_renal/WSI_tile%A_%a.log                 # File to which STDOUT will be written, including job ID (%j)
#SBATCH -e ./logs/tile_DFCI_renal/WSI_tile%A_%a.log                 # File to which STDERR will be written, including job ID (%j)
#SBATCH --array=0-636          # Run array for indexes. Should set to the length of the file list
...
FILE_LIST="data_sheet/DFCI_renal(N=636).csv"
...
PROJ="DFCI_renal"                ## output subfolder name. the tiles will be stored in [outpath]/[proj]/[slide_name]
