...
class PathologyDataset(Dataset):
    ...
    def openImg(self, path, transform = None):
        if os.path.isfile(path) is True:
            ...
            try:
                ...
                exit()
            if transform == None: ## if transform is None, using self.image_padding Line. 74
    ...
    def pickPatches(self, sampleDirectory, folder):
        if folder not in self.folderDict: 
            ...
            if self.roi == True: ## we use roi file to choose images
                listPatches = self.pklPatchesInformation.loc[self.pklPatchesInformation["folder_id"].isin([folder]) & self.pklPatchesInformation['height'].isin([256]) & self.pklPatchesInformation['width'].isin([256])]\
                                        .sort_values(by=["weight"], ascending = False).head(self.patchesPerBag*self.multiply)["figure_id"].tolist()
            else:
                if self.noise == False: 
                    listPatches = self.pklPatchesInformation.loc[self.pklPatchesInformation["folder_id"].isin([folder]) & self.pklPatchesInformation['height'].isin([256]) & self.pklPatchesInformation['width'].isin([256])]\
                                            .sort_values(by=["foreground_background_percentage"], ascending = False).head(self.patchesPerBag*self.multiply)["figure_id"].tolist()
                else: ## choose images whose size is smaller than size.
                    listPatches = self.pklPatchesInformation.loc[self.pklPatchesInformation["folder_id"].isin([folder])]\
                                            .sort_values(by=["foreground_background_percentage"], ascending = False).head(self.patchesPerBag*self.multiply)["figure_id"].tolist()
